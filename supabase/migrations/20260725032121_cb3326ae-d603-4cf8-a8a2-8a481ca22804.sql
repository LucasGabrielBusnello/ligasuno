...
AS $$
  ...
  WHERE (
    ...
    OR EXISTS (SELECT 1 FROM public.leagues l WHERE l.id = _league_id AND l.president_id = auth.uid())
...
GRANT EXECUTE ON FUNCTION public.find_profile_for_league(uuid, text) TO authenticated;

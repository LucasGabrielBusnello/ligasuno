...
function PresidentePage() {
  ...
  if (!league || !user || !accessChecked) return <div className="p-12 text-center">Carregando...</div>;
  const isOwner =
            ...
            <div className="rounded border p-3 space-y-2">
              ...
              {!regSelected && regResults !== null && regResults.length === 0 && regBusy !== "search" && (
                <p className="text-[11px] text-amber-600 dark:text-amber-400">
                  Nenhum usuário encontrado com esse termo. A pessoa precisa ter conta no MEDPLEX para ser inscrita — peça o cadastro com o e-mail dela e busque novamente.
                </p>
              )}
...
function MembersTab({ league }: any) {
  ...
  async function add() {
    ...
    if (!prof) {
      ...
      if (!profs || profs.length === 0) return toast.error("Nenhum usuário encontrado com esse e-mail/usuário. Confirme se a pessoa já criou uma conta no MEDPLEX.");
